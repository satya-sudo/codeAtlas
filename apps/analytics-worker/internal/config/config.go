...
func Load() (Config, error) {
	...
	pollInterval, err := sharedconfig.GetDuration("ANALYTICS_WORKER_POLL_INTERVAL", 15*time.Second)
	if err != nil {
		return Config{}, err
	}

	shutdownTimeout, err := sharedconfig.GetDuration("ANALYTICS_WORKER_SHUTDOWN_TIMEOUT", 10*time.Second)
	if err != nil {
		return Config{}, err
	}
